...
void Queue::add_to_back(Location loc) {
   ...
   contents[tail] = loc; //new location at tail
   tail++;  //one more location
...
Location Queue::remove_from_front() {
   ...
   // *** You complete **** CHECKPOINT 3
   Location x = contents[head];
   head++;   //one more location extracted 
   return x;  //returns oldest location, first element 
