...
for (int i = 0; i < width; i++){
  for (int j = 0; j < height; j++){  //iterate through each pixel
...
void gray2binary(unsigned char **in,unsigned char **out,int height,int width) {
  ...
  for (int i = 0; i < height; i++){
    for (int j = 0; j < width; j++){
      if (in[i][j] > THRESHOLD){  //if greater than threshold make white
        out[i][j] = 1;
      }
      else out[i][j] = 0;
    }
...
int component_labeling(unsigned char **binary_image,int **label,int height,int width) {
  //step 1
  for (int i = 0; i < height; i++){
    for (int j = 0; j < width; j++){  //initialize label
      label[i][j] = 0;
    }
  }
  //step 2
  int current_label = 0;
  //step 3
  unsigned char **exploredArray = 0; //declaration
  exploredArray = new unsigned char*[height]; //dynamically allocated
  for (int i = 0; i < height; i++){
    exploredArray[i] = new unsigned char[width];
    for (int j = 0; j < width; j++){
      exploredArray[i][j] = 0;  //intialized
    }
  }

  Location pixel;  //pixel location
  Location current;  //current location
  for (int i = 0; i < height; i++){
    for (int j = 0; j < width; j++){
      pixel.row = i; pixel.col = j;
      if (binary_image[pixel.row][pixel.col] == 1){ //if white
        if (exploredArray[pixel.row][pixel.col] == 0){ //if non explored
          Queue q(height*width); //new queue, q
          current_label++;
          exploredArray[pixel.row][pixel.col] = 1; //make explored 
          label[pixel.row][pixel.col] = current_label;
          q.push(pixel);  //push it to bac
          Location West, East, South, North;  //neighbors 
          while (!(q.is_empty())){
            current = q.pop();
            West = current; East = current; South = current; North = current;
            West.row = current.row; West.col = current.col-1;  //BFS, same as PA3
            if (West.row >= 0 && West.row < height &&
            West.col >= 0 && West.col < width){
              if (binary_image[West.row][West.col] == 1){
                if (exploredArray[West.row][West.col] == 0){
                  exploredArray[West.row][West.col] = 1;
                  label[West.row][West.col] = current_label;
                  q.push(West);
                }
              }
            }
            East.row = current.row; East.col = current.col+1;
            if (East.row >= 0 && East.row < height &&
            East.col >= 0 && East.col < width){
              if (binary_image[East.row][East.col] == 1){
                if (exploredArray[East.row][East.col] == 0){
                  exploredArray[East.row][East.col] = 1;
                  label[East.row][East.col] = current_label;
                  q.push(East);
                }
              }
            }
            North.row = current.row-1; North.col = current.col;
            if (North.row >= 0 && North.row < height &&
            North.col >= 0 && North.col < width){
              if (binary_image[North.row][North.col] == 1){
                if (exploredArray[North.row][North.col] == 0){
                  exploredArray[North.row][North.col] = 1;
                  label[North.row][North.col] = current_label;
                  q.push(North);
                }
              }
            }
            South.row = current.row+1; South.col = current.col;
            if (South.row >= 0 && South.row < height &&
            South.col >= 0 && South.col < width){
              if (binary_image[South.row][South.col] == 1){
                if (exploredArray[South.row][South.col] == 0){
                  exploredArray[South.row][South.col] = 1;
  ...
  for (int i = 0; i < height; i++){  //deallocate explored
...
void label2RGB(int  **labeled_image, unsigned char ***rgb_image,int num_segments,int height,int width)
{
  ...
  for (int i = 1; i <= num_segments; i++){
    unsigned char randomRed = (unsigned char)rand() % 256;
    unsigned char randomGreen = (unsigned char)rand()%256;  //gen random colors for rgb
    unsigned char randomBlue = (unsigned char)rand()%256;
    for (int r = 0; r < height; r++){
      for (int c = 0; c < width; c++){
        if (labeled_image[r][c] == i){
          rgb_image[r][c][0] = randomRed;  //assign random rgb colors to segments
          rgb_image[r][c][1] = randomGreen;
          rgb_image[r][c][2] = randomBlue;
        }
      }
    }
  }
...
void clean(unsigned char ***input,unsigned char **gray,unsigned char **binary, int **labeled_image,int height , int width) {
    if(input) {
      ...
      for (int i = 0; i < width; i++){
        for (int j = 0; j < width; j++){  //deallocation of 3d arr
          delete [] input[i][j];
        }
        delete [] input[i];
      }
    ...
    if(gray){
      ...
      for (int i = 0; i < width; i++){  //deallocation of 2d arr
        delete [] gray[i];
      }
      delete [] gray;
